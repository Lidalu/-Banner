...
typedef NS_ENUM(NSUInteger, LLuBannerScrollType) {
    LLuBannerScrollTypeStatic, // 非滑动状态
    LLuBannerScrollTypeLeft,
    LLuBannerScrollTypeRight
};

@interface LLuBannerView () <UIScrollViewDelegate>
...
// 滚动状态
@property (nonatomic, assign) LLuBannerScrollType scrollType;
// 当前显示图片的Index
@property (nonatomic, assign) NSInteger currentImageIndex;

@end

@implementation LLuBannerView
{
    NSTimer * _timer;
}
...
- (UILabel *)descLabel {
    ...
    if (!_descLabel) {
        ...
        _descLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 180, self.frame.size.width, 20)];
        _descLabel.backgroundColor = [UIColor blackColor];
        _descLabel.alpha = 0.5;
        [_descLabel setFont:[UIFont systemFontOfSize:17]];
        _descLabel.textColor = [UIColor whiteColor];
    }
    return _descLabel;
}

- (NSArray *)descTitileList {
    
    if (!_descTitileList) {
        
        _descTitileList = [Banner bannersList];
    }
    return _descTitileList;
}

+ (instancetype)bannerView {
    return [[self alloc] initWithFrame:CGRectMake(0, 64, 414, 200)];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self setupViews];
        NSLog(@"pageControl->>>%@", NSStringFromCGRect(self.pageControl.frame));
...
- (void)layoutSubviews {
    ...
    self.pageControl.center = CGPointMake(self.frame.size.width - 45, CGRectGetMaxY(self.scrollView.frame) - 10);
...
- (void)loadDisplayImage {
    ...
    Banner *banner = self.descTitileList[self.currentImageIndex];
    self.descLabel.text = banner.title;
}

- (void)loadReuseImage {
    CGRect frame = self.reuseImageView.frame;
    NSInteger reuseIndex = self.currentImageIndex;
    if (self.scrollType == LLuBannerScrollTypeLeft) {
        frame.origin.x = 0;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (![self isInScreen:self.displayImageView.frame]) {
        ...
        // 还原滚动方向
        self.scrollType = LLuBannerScrollTypeStatic;
        // 更新Display Image
        [self loadDisplayImage];
        ...
        // 右
        if (self.scrollType != LLuBannerScrollTypeRight) {
            self.scrollType = LLuBannerScrollTypeRight;
            [self loadReuseImage];
        }
    } else {
        // 左
        if (self.scrollType != LLuBannerScrollTypeLeft) {
            self.scrollType = LLuBannerScrollTypeLeft;
            [self loadReuseImage];
        }
    }
}
